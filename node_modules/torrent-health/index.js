var torrent = { read: require('read-torrent') }
  , Q       = require('q')
  , async   = require('async')
  , util    = require('util')
  , LOG     = require('debug')('torrent-health')
  , Tracker = require('node-tracker');

module.exports = TorrentHealth;
...
function TorrentHealth(uri, options) {
    if(!options) options = {};
    ...
    torrent.read(uri, function(err, info) {
        if(err) {
            ...
            async.map(info.announce, function(trUri, done) {
                ...
                tracker.scrape([info.infoHash], {timeout: options.timeout}, function(err, data) {
                    if(err) {
                        if(err.message === 'timed out' || err.code === 'ETIMEDOUT') {
                            LOG('Scrape timed out for ' + trUri);
                            return done(null, null);
                        } else {
                            LOG('Error in node-tracker: ' + err.message);
                ...
                if(err) {
                    ...
                    var totalSeeds = 0, totalPeers = 0, total = 0;
                    ...
                    return defer.resolve({
                        seeds: totalSeeds / total | 0,
                        peers: totalPeers / total | 0
                    });
                }
            });
        }
